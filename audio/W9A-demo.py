# W9A-demo.py: Normalize, Trim Silence, 2x Speed Up: LICENSE = ?
import numpy as np
from pedalboard.io import AudioFile

# trim starting and ending silence from the audio clip
def trim_silence(audio):
    chunk_size = 1024 # process in 1024-sample chunks (23ms at 44.1 KHz)
...
def change_speed(audio, rate):
    indices = (np.arrange(0, len(audio) - 1, rate)).astype(int)
